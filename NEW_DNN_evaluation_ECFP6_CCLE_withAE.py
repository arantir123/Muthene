...
# the implementation of the DeepSynergy using ECFP6 + selected 60 cell lines (described by CCLE gene expression data) with adverse effects
...
def run_model_DNN_DDI(root_prefix, num_epochs, patience, batch_size, repeat, hidden_dim_aux, layer_list, dropout, input_dropout, whether_CCLE):
    ...
    for _ in range(repeat):
        ...
        main_net = DNN_predictor(cellline_expression, all_drug_morgan.shape[1]*2 + len(se_symbol2id_dict), len(cellline2id_dict), hidden_dim_aux, layer_list, dropout, input_dropout, whether_CCLE)
        main_net.to(device)
        ...
        if only_test==False:
            for epoch in range(num_epochs):
                ...
                for iteration in range(train_sample_idx_generator.num_iterations()):
                    ...
                    te_output = main_net(row_drug_struc_embedding, col_drug_struc_embedding, train_cellline_idx, train_se_labels_batch) # add adverse effect labels
                ...
                with torch.no_grad():
                    for iteration in range(val_sample_idx_generator.num_iterations()):
                        ...
                        val_se_labels_batch_combined = torch.cat([val_se_labels_batch, val_se_labels_batch], axis=0)
                        # double the adverse effect labels as the part of the input of reverse drug pair samples
                        # forward and reverse direction drug pair samples share the same adverse effect labels
                        ...
                        row_drug_struc_embedding, col_drug_struc_embedding = all_drug_morgan[row_drug_batch], all_drug_morgan[col_drug_batch]
                        te_output = main_net(row_drug_struc_embedding, col_drug_struc_embedding, val_cellline_idx, val_se_labels_batch_combined) # add adverse effect labels
                ...
                t_end = time.time()
                print('Epoch {:05d} | Val_Loss {:.4f} | Time(s) {:.4f}'.format(epoch, val_total_loss.item(), t_end - t_start))
        ...
        with torch.no_grad():
            for iteration in range(test_sample_idx_generator.num_iterations()):
                ...
                test_se_labels_batch_combined = torch.cat([test_se_labels_batch, test_se_labels_batch], axis=0)
                ...
                row_drug_struc_embedding, col_drug_struc_embedding = all_drug_morgan[row_drug_batch], all_drug_morgan[col_drug_batch]
                te_output = main_net(row_drug_struc_embedding, col_drug_struc_embedding, test_cellline_idx, test_se_labels_batch_combined)
                te_output = (te_output[:te_output.shape[0] // 2, :] + te_output[te_output.shape[0] // 2:, :]) / 2
...
if __name__ == '__main__':
    # part1
    ap = argparse.ArgumentParser(description='DeepSynergy testing for drug-drug link prediction')
