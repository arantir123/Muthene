...
import numpy as np
from model.Auxiliary_networks import side_effect_predictor, therapeutic_effect_DNN_predictor, GIN4drug_struc
from model.HNEMA_link_prediction import HNEMA_link_prediction
from utils.pytorchtools import EarlyStopping
from utils.data import load_HNEMA_DDI_data_te, DrugStrucDataset # DrugStrucDataset has a self-contained path indicating the path for reading current nodes/edge/properties of molecular graphs
...
# the implementation of the Muthene variant using GIN (encoding drug molecular graphs) + selected 60 cell lines (described by CCLE gene expression data) with adverse effect module
...
def run_model_HNEMA_DDI(root_prefix, hidden_dim_main, num_heads_main, attnvec_dim_main, rnn_type_main,
    ...
    # read DGLdataloader storing drug molecular structures
    drug_info_set = DrugStrucDataset()
    # load molecular graphs for all drugs
    ...
    for _ in range(repeat):
        ...
        te_net = therapeutic_effect_DNN_predictor(cellline_expression, len(cellline2id_dict),
                                                  hidden_dim_main + hidden_dim_aux, hidden_dim_aux, te_layer_list,
        ...
        tot_params4 = sum([np.prod(p.size()) for p in drug_net.parameters()])
        print(f"Total number of parameters in drug_net: {tot_params4}")
        print(f"Total number of parameters in Muthene: {tot_params1 + tot_params2 + tot_params3 + tot_params4}")
        ...
        if only_test == False:
            for epoch in range(num_epochs):
                ...
                for iteration in range(train_sample_idx_generator.num_iterations()):
                    ...
                    row_drug_batch, col_drug_batch = np.array(train_drug_drug_idx)[:, 0], np.array(train_drug_drug_idx)[:, 1] # drug pair ids
                    drug_struc_input, drug_idx = iter(drug_struc_loader).next() # read all molecular graphs
                    ...
                    drug_struc_embedding = drug_net(drug_struc_input, drug_atom_num) # generate drug embedding look-up table
                    row_drug_struc_embedding, col_drug_struc_embedding = drug_struc_embedding[row_drug_batch], drug_struc_embedding[col_drug_batch] # retrieve required embeddings
                    row_drug_composite_embedding = torch.cat((row_drug_embedding, row_drug_struc_embedding), axis=1) # concatenate with meta-path embeddings
...
if __name__ == '__main__':
    ...
    ap = argparse.ArgumentParser(description='Muthene GIN variant testing for drug-drug link prediction')
    ...
    ap.add_argument('--hidden-dim-aux', type=int, default=64,
                    help='Dimension of generated cell line and drug structural embeddings. Default is 64.')
    ...
    ap.add_argument('--loss-ratio-te', type=float, default=0.1,
